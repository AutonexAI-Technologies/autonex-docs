...
export const step1Schema = z.object({
  ...
  phone: z.string().optional().default('').transform(v => normalizePhone(v ?? '')).refine(
    val => val === '' || /^\+[1-9]\d{6,14}$/.test(val),
    'Enter a valid international phone number (e.g. +91 9876543210)'
  ),

  company: z.string().max(100, 'Company name must be under 100 characters').optional().default(''),
...
export const step2Schema = z.object({
  ...
  // Accept string OR number (form sends number, API also fine with string)
  total_fee: z.union([
    z.string().min(1, 'Total fee is required').refine(
      v => !isNaN(Number(v)) && Number(v) > 0,
      'Fee must be greater than 0'
    ),
    z.number().positive('Fee must be greater than 0'),
  ]),

  start_date: z.string().optional().default(''),
  project_type: z.enum(['one-time', 'retainer']).default('one-time'),
})

// ─── Step 3 — Payment Details ───────────────────────────
// All optional — but if provided, must be valid
export const step3Schema = z.object({
  bank_name: z.string().optional().default('').refine(
    val => {
...
/** Validate a whole step and return field-level errors */
