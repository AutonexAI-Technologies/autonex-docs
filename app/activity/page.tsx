...
const entityColor: Record<string, string> = {
  ...
  settings: 'text-[var(--text-tertiary)] bg-slate-500/10',
}
    ...
    <div className="px-6 py-8 max-w-[1400px] mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between flex-wrap gap-4 mb-8">
        <div>
          ...
          <p className="text-[var(--text-tertiary)] text-sm mt-1">Every action is permanently logged. Cannot be edited or deleted.</p>
        </div>
        <div className="relative">
          ...
          <Input
            ...
            className="pl-9 h-9 w-56 bg-slate-50 border-slate-200 text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] text-sm rounded-xl focus:border-blue-500/50"
          />
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}
        className="mb-6 flex items-center gap-3 px-4 py-3 rounded-xl border border-emerald-500/15 bg-emerald-500/5 text-[var(--text-tertiary)] text-xs"
      >
        <span>🔒</span>
      ...
      >
        <div className="hidden md:grid grid-cols-[auto_2fr_2fr_1fr_1fr] gap-4 px-6 py-3 border-b border-slate-200 text-xs text-[var(--text-tertiary)] uppercase tracking-wider">
          <span className="w-8" />
          <span>Action</span>
          <span>Entity</span>
          <span>By</span>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="w-14 h-14 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-center mb-4">
              <Activity className="w-6 h-6 text-[var(--text-secondary)]" />
            </div>
            <p className="text-white font-medium">No activity yet</p>
            <p className="text-[var(--text-tertiary)] text-sm mt-1">Actions will appear here as you use the platform</p>
          </div>
        ) : (
          <div className="divide-y divide-slate-100">
            {filtered.map((log, i) => {
              const Icon = entityIcon[log.entity_type] || Activity
              const colorClass = entityColor[log.entity_type] || 'text-[var(--text-tertiary)] bg-slate-500/10'
              return (
                <motion.div
                  key={log.id}
                  initial={{ opacity: 0 }}
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-[auto_2fr_2fr_1fr_1fr] gap-4 items-center px-6 py-4 hover:bg-slate-50 transition-colors">
                    ...
                    <p className="hidden md:block text-[var(--text-tertiary)] text-xs">{log.entity_name || log.entity_type}</p>
